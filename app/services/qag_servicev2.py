...
class QAGService(AbstractService):
    def __init__(self, supabase: Client, payload: QAGRequest):
        super().__init__(
            ...
            BUCKET,
            "qag",
            "form_qualidade_da_agua_superficial",
            QAGResponse,
            QAGRequest,
        )
    ...
    def gerar_relatorio(self) -> QAGResponse:
        ...
        q_20_6 = lab0["email"]
        q_20_7 = lab0["contato"]
        q_25 = self.configuracoes.data[0]["dados_laboratoriais"][0].get("metodologia_adotada")
        q_26 = indicadores_qag

        sel27 = self.df_resultados[["Ponto", "Profundidade"] + self.parametros].to_dict(orient="records")
        self.inserir_tabela_agrupada(data=sel27, parametros=self.parametros, ctx_key="tabela_qag_27")
        ...
        for _, row in self.df_resultados.iterrows():
            ...
            for p in self.parametros:
                ...
                registros.append({"Parametro": p, "Valor": val, "Conforme": conform})
        df_comparacao = pd.DataFrame(registros)
        q_31 = df_comparacao["Conforme"].mean() * 100

        self.inserir_tabela_media(df=self.df_resultados, parametros=self.parametros, ctx_key="QAG_32")

        pm = self.parametros_metais_pesados
        regs34 = self.df_resultados[["Ponto", "Profundidade"] + pm].to_dict(orient="records")
        self.inserir_tabela_agrupada(data=regs34, parametros=pm, ctx_key="QAG_34")
        q_35 = self._percentual_conformidade(self.df_resultados, pm, vmp_qag)
        figs36 = []
        for cls in self.df_resultados["Classe"].unique():
            figs36.extend(
                grafico_qualidade_agua(
        ...
        self.inserir_tabela_media(df=self.df_resultados, parametros=pm, ctx_key="QAG_37")

        sol = self.solventes
        q_38 = self._percentual_conformidade(self.df_resultados, sol, vmp_qag)
        self.inserir_tabela_media(df=self.df_resultados, parametros=sol, ctx_key="QAG_39")
        regs40 = self.df_resultados[["Ponto", "Profundidade"] + sol].to_dict(orient="records")
        self.inserir_tabela_agrupada(data=regs40, parametros=sol, ctx_key="QAG_40")
        ...
        tabela47["Resultado"] = tabela47["Conforme"].map({True: "Alcançado", False: "Não Alcançado"})
        tabela47 = tabela47.drop(columns=["Tipo", "Programa", "Valor", "Unidade", "Conforme"])
        ...
        self.contexto.update(
            {
                ...
                "QAG_09": self.ativo.data[0]["nome"],
                "QAG_10": self.ativo.data[0]["cnpj"],
                "QAG_11": self.ativo.data[0]["endereco"],
                "QAG_12": self.ativo.data[0]["nome"],
                "QAG_13": self.ativo.data[0]["numero_licenca"],
                "QAG_14": q_14,
                "QAG_15": self.ativo.data[0]["orgao_regulador"],
                "QAG_16": self.ativo.data[0]["endereco"],
                "QAG_18": self.configuracoes.data[0]["localizacao_dos_pontos_de_monitoramento"],
                "QAG_19": self.configuracoes.data[0]["parametro_periodicidade"],
                "QAG_20_1": q_20_1,
                "QAG_20_2": q_20_2,
                "QAG_20_3": q_20_3,
                "QAG_20_4": q_20_4,
